...
begin
  ...
  if isundefined(lhs_m.src) then
    undefine rhs_m.src;
  else
    rhs_m.src := map_LHS_to_RHS(lhs_m.src);
  endif;
  if isundefined(lhs_m.dst) then
    undefine rhs_m.dst;
  else
...
begin
  ...
  if isundefined(lhs_m.src) & !isundefined(rhs_m.src) then
    return false;
  elsif !isundefined(lhs_m.src) & isundefined(rhs_m.src) then
    return false; 
  elsif !isundefined(lhs_m.src) & !isundefined(rhs_m.src)  then
  ...
  if isundefined(lhs_m.dst) & !isundefined(rhs_m.dst) then
    return false;
  elsif !isundefined(lhs_m.dst) & isundefined(rhs_m.dst) then
    return false; 
  elsif !isundefined(lhs_m.dst) & !isundefined(rhs_m.dst)  then
